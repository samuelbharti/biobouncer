...
# UniParc endpoint; the biobouncer.remote_transport option replaces it.
...
test_that("an existing sequence is valid", {
  ...
  withr::local_options(biobouncer.remote_transport = .stub_uniparc(200))
...
test_that("an absent sequence is invalid", {
  withr::local_envvar(BIOBOUNCER_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biobouncer.remote_transport = .stub_uniparc(404))
...
test_that("an unexpected status raises and is not cached", {
  withr::local_envvar(BIOBOUNCER_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biobouncer.remote_transport = .stub_uniparc(500))
  expect_error(
