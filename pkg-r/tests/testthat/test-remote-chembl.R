...
# id-lookup endpoint; the biogate.remote_transport option replaces it.
...
test_that("an existing entity is valid", {
  ...
  withr::local_options(biogate.remote_transport = .stub_chembl(200))
...
test_that("an absent entity is invalid", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_chembl(404))
...
test_that("an unexpected status raises and is not cached", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_chembl(500))
  expect_error(
