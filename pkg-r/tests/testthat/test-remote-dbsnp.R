...
# against the NCBI RefSNP API; the biobouncer.remote_transport option replaces it.
...
test_that("a current rsID is valid", {
  ...
  withr::local_options(biobouncer.remote_transport = .stub_dbsnp(200))
...
test_that("a merged rsID is invalid and suggests the primary id", {
  withr::local_envvar(BIOBOUNCER_CACHE_DIR = withr::local_tempdir())
  withr::local_options(
    biobouncer.remote_transport = .stub_dbsnp(200, .merged_body)
  )
...
test_that("an absent rsID is invalid", {
  withr::local_envvar(BIOBOUNCER_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biobouncer.remote_transport = .stub_dbsnp(404))
...
test_that("an unexpected status raises and is not cached", {
  withr::local_envvar(BIOBOUNCER_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biobouncer.remote_transport = .stub_dbsnp(500))
  expect_error(
