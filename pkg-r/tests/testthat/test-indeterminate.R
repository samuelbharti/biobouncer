...
# test transport aborts with class biogate_error_remote to stand in for a network
# failure that has exhausted its retries.

.fail_transport <- function(url, timeout) {
  cli::cli_abort("connection refused", class = "biogate_error_remote")
}
...
test_that("the default raises on a remote failure", {
  ...
  withr::local_options(biogate.remote_transport = .fail_transport)
  expect_error(
...
test_that("indeterminate isolates the failure to one id", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .mixed_transport)
  res <- check_id(
