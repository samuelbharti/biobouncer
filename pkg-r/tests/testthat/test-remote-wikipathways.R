...
# published WikiPathways asset; the biobouncer.remote_transport option replaces it.
...
test_that("an existing pathway is valid", {
  ...
  withr::local_options(biobouncer.remote_transport = .stub_wikipathways(200))
...
test_that("an absent pathway is invalid", {
  withr::local_envvar(BIOBOUNCER_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biobouncer.remote_transport = .stub_wikipathways(404))
...
test_that("an unexpected status raises and is not cached", {
  withr::local_envvar(BIOBOUNCER_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biobouncer.remote_transport = .stub_wikipathways(500))
  expect_error(
