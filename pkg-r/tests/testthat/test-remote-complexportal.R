...
# EBI Complex Portal web service; the biogate.remote_transport option replaces it.
...
test_that("an existing complex is valid", {
  ...
  withr::local_options(biogate.remote_transport = .stub_complexportal(200))
...
test_that("an absent complex is invalid", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_complexportal(404))
...
test_that("an unexpected status raises and is not cached", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_complexportal(500))
  expect_error(
