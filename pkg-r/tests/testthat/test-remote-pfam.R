...
# InterPro API, which hosts Pfam; the biogate.remote_transport option replaces it.
...
test_that("an existing family is valid", {
  ...
  withr::local_options(biogate.remote_transport = .stub_pfam(200))
...
test_that("an absent family is invalid", {
  ...
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_pfam(204))
...
test_that("an unexpected status raises and is not cached", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_pfam(500))
  expect_error(
