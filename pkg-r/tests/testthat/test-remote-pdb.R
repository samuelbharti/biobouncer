...
# data API; the biogate.remote_transport option replaces it.
...
test_that("an existing structure is valid", {
  ...
  withr::local_options(biogate.remote_transport = .stub_pdb(200))
...
test_that("an absent structure is invalid", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_pdb(404))
...
test_that("an unexpected status raises and is not cached", {
  withr::local_envvar(BIOGATE_CACHE_DIR = withr::local_tempdir())
  withr::local_options(biogate.remote_transport = .stub_pdb(500))
  expect_error(
