...
"is this a valid identifier?", with the same verdict in R, Python, and
JavaScript. This
...
# existence mode uses a snapshot when one answers, otherwise remote, and
# pattern for a source with no resolver.
...
- Results are vectorized and preserve input order and length. Errors are
  explicit.
- The same inputs give the same verdicts in the Python and JavaScript
  packages, which a shared conformance corpus enforces.
