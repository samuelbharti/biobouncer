...
biobouncer checks 50 sources. Each table below gives a valid example id, the
checking modes a source supports, and whether it is species-aware. To check an
id, pass the source key as `source_db`. The call is the same for every source:
...
pinned snapshot, and `remote` checks existence against the live source. See
...
```r
is_valid_id("HP:0001250", source_db = "hp")
```
...
```r
is_valid_id("UBERON:0002107", source_db = "uberon")
```
...
```r
is_valid_id("NCBITaxon:9606", source_db = "ncbitaxon")
```
...
```r
is_valid_id("TP53", source_db = "hgnc")
```
...
```r
is_valid_id("rs7412", source_db = "dbsnp")
```
...
```r
is_valid_id("PF00001", source_db = "pfam")
```
...
```r
is_valid_id("GO:0006915", source_db = "go")
```
...
```r
is_valid_id("CHEBI:15377", source_db = "chebi")
```
...
```r
is_valid_id("R-HSA-68886", source_db = "reactome")
```
