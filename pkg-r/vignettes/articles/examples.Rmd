...
pointblank agents run an expression per column. `is_valid_id()` is exactly the
right shape, so it drops into a `col_vals_expr()` step. This needs the
...
`sv_biobouncer()` returns a shinyvalidate rule. It returns `NULL` for a valid input
