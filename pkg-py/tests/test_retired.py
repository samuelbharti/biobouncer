...
from biobouncer._cache import _snapshot_retired
from biobouncer._remote import _normalize_obo
...
def test_remote_obsolete_suggests_successor(monkeypatch):
    ...
    res = biobouncer.check_id("GO:0006917", source_db="go", how="remote")[0]
...
def test_remote_obsolete_cross_ontology_successor(monkeypatch):
    ...
    res = biobouncer.check_id("EFO:0000401", source_db="efo", how="remote")[0]
    assert res.valid is False
    assert res.normalized is None
...
def test_cache_retired_suggests_successor():
    res = biobouncer.check_id(
        "GO:0006917", source_db="go", how="cache", version="sample"
    )[0]
    assert res.valid is False
    assert res.normalized is None
    assert res.suggestion == "GO:0006915"
...
def test_cache_not_retired_has_no_suggestion():
    res = biobouncer.check_id(
        "GO:0000001", source_db="go", how="cache", version="sample"
    )[0]
    assert res.valid is False
