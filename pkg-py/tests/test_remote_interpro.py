...
def test_existing_entry_is_valid(monkeypatch):
    ...
    res = biobouncer.check_id("IPR000001", source_db="interpro", how="remote")[0]
...
def test_absent_entry_is_invalid(monkeypatch):
    ...
    res = biobouncer.check_id("IPR999999", source_db="interpro", how="remote")[0]
...
def test_malformed_skips_the_network(monkeypatch):
    ...
    res = biobouncer.check_id("IPRABCDEF", source_db="interpro", how="remote")[0]
    assert res.valid is False
    assert res.suggestion is None
...
def test_unexpected_status_raises_and_is_not_cached(monkeypatch):
    ...
    with pytest.raises(RemoteError):
        biobouncer.check_id("IPR000001", source_db="interpro", how="remote")
