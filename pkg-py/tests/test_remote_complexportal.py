...
def test_existing_complex_is_valid(monkeypatch):
    ...
    res = biobouncer.check_id("CPX-2158", source_db="complexportal", how="remote")[0]
...
def test_absent_complex_is_invalid(monkeypatch):
    ...
    res = biobouncer.check_id("CPX-9999999", source_db="complexportal", how="remote")[0]
...
def test_malformed_skips_the_network(monkeypatch):
    ...
    res = biobouncer.check_id("CPX-XYZ", source_db="complexportal", how="remote")[0]
    assert res.valid is False
    assert res.suggestion is None
...
def test_unexpected_status_raises_and_is_not_cached(monkeypatch):
    ...
    with pytest.raises(RemoteError):
        biobouncer.check_id("CPX-2158", source_db="complexportal", how="remote")
